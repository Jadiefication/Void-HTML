    ...
    fun `test special route handler in RouterUtil`() {
        ...
        val mainPage = route("/main") {
            GET {
                ...
                io.voidx.html.util.createResponse(el)
            }
        }
        router.addRoute(mainPage)

        // 2. Setup a KtsPage
        val ktsPage = ktsRoute("/kts-endpoint") { req, trigger, target ->
        ...
        val request3 = buildRequest {
            target = "/main"
        }
        
        val invokeMethod = handlerLambda::class.java.methods.first { 
