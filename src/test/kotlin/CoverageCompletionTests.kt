...
class CoverageCompletionTests {
    ...
    fun element_getName_coverage() {
        val el = object : Element("custom-name") {
            override fun render() = ""
        }
    ...
    fun js_page_addToMetadata_existing_meta_null_externalJS() {
        val page = route("/") { }
        page.metadata = io.voidx.html.metadata.Metadata(page)
        val jsPage = io.voidx.html.page.JsPage(UUID.randomUUID(), "console.log(1)")
    ...
    fun element_findElement_no_prefix() {
        val el = object : Element("el") { override fun render() = "" }
    ...
    fun kts_page_getters() {
        val ktsPage = object : KtsPage("/kts") {
